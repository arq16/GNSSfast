...
#' @param selection.K a name indicating the model selection criterion to select the number of segments K (\code{mBIC}, \code{Lav}, \code{BM_BJ} or \code{BM_slope}). \code{"none"} indicates that no selection is claimed and the procedure considers \code{Kmax} segments or \code{Kmax}-1 changes. If \code{selection.K="All"}, the results for the four possible criteria are given. Default is \code{"BM_BJ"}
...
#' Kmax=4
#' lmin=1
#' result=GNSSfast(Data,lyear,Kmax=Kmax,selection.K="none")
#' plot_GNSS(Data,result$seg,result$funct)
#' @export

GNSSfast=function(Data,lyear=365.25,lmin=1,Kmax=30,selection.K="BM_BJ",S=0.75,f=TRUE,selection.f=FALSE,threshold=0.001,tol=1e-4){
  result  <-  list()
  Data.X  <-  c()
  cond1 <- TRUE
  cond2 <- TRUE
  ...
  if (class(Data$date)[1]!="POSIXct"){
    cond1=FALSE
    warning("date must be in format yyyy-mm-dd of type GMT in class POSIXct/POSIXt")
  }
  if (Kmax >n.X) {
    cond2=FALSE
  ...
  if ((cond1==TRUE) & (cond2==TRUE) ){
    ...
    if (f==TRUE){
      ...
      if (selection.K=="none"){
        res.segfunct=c()
        request=paste(paste0("res.segfunct=",Used.function,'(Data.X,var.est.month,Kmax,myGraph[[Kmax]],lmin,lyear,threshold,tol)'),sep="")
      ...
      if (selection.K=="Lav"){
        res.LoopK=Loop.K.procedure(Data.X,var.est.month,lyear,lmin,Kmax,myGraph,Used.function,threshold,tol)
      ...
      res.LoopK=Loop.K.seg.procedure(Data.X,var.est.month,lyear,lmin,Kmax,myGraph)
      ...
      if (selection.K=="none"){
        res.seg.with.NA<- add_NA(res.LoopK[[Kmax]],present.data,n.Data,segf=FALSE)
